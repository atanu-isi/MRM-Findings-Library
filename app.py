...
import collections as _collections, threading as _threading, datetime as _datetime

_log_lock   = _threading.Lock()
_log_buffer = _collections.deque(maxlen=200)   # last 200 lines
...
# ── Ollama helpers ────────────────────────────────────────────────────────────
import os as _os, urllib.request as _urlreq, socket as _socket

def _ollama_hosts():
    """Return candidate Ollama base-URLs to try, in order."""
    env = _os.environ.get('OLLAMA_HOST', '').rstrip('/')
    candidates = []
    if env:
        candidates.append(env)
    # Always try both localhost variants — whichever one Ollama bound to
...
def _ollama_model():
    return _os.environ.get('OLLAMA_MODEL', 'tinyllama:latest')

def _try_ollama_chat(host, model, messages, timeout=180):
    """
    POST to Ollama /api/chat.  Returns (text, None) on success or
    (None, error_string) on failure.  All activity is written to _log_buffer.
    """
    _log(f'REQUEST  model={model}  host={host}  msgs={len(messages)}')
    payload = json.dumps({
        'model':    model,
        'messages': messages,
        'stream':   False,
    ...
    try:
        ...
        if not text:
            _log('EMPTY    Ollama returned no content — model may still be loading')
            return None, 'Ollama returned an empty response — model may still be loading, retry in a moment.'
        stats = []
        for key in ('eval_count', 'prompt_eval_count', 'total_duration'):
            if key in data:
                stats.append(f'{key}={data[key]}')
        _log(f'OK       {len(text)} chars returned' + (f'  [{", ".join(stats)}]' if stats else ''))
        return text, None
...
def ollama_check():
    ...
    in your browser to see which Ollama URL is reachable and which models
    are loaded.
    """
    results = {}
    for host in _ollama_hosts():
        try:
            with _urlreq.urlopen(f'{host}/api/tags', timeout=4) as r:
...
# ── Logs endpoint: returns buffered log lines ─────────────────────────────────
@app.route('/api/ollama-logs', methods=['GET'])
def ollama_logs():
    """
    Returns last N log lines from Ollama interactions plus a connectivity status.
    Polled every 2 s by the frontend Logs panel.
    """
    # Quick reachability ping
    status = 'error'
    for host in _ollama_hosts():
        try:
            with _urlreq.urlopen(f'{host}/api/tags', timeout=2) as r:
                r.read()
            status = 'ok'
            break
        except Exception:
...
def llm_proxy():
    """
    Accepts: { "system": "...", "user": "..." }
    Tries each candidate Ollama host in turn and returns { "text": "..." }.
    Override via env vars OLLAMA_HOST and OLLAMA_MODEL.
    """
    ...
    last_errors = {}
    for host in _ollama_hosts():
        _log(f'PROXY    trying host {host} model {model}')
        text, err = _try_ollama_chat(host, model, messages)
        if text:
            _log(f'PROXY    success via {host}')
            return jsonify({'text': text})
        last_errors[host] = err
        _log(f'PROXY    failed {host}: {err}')
        # Only move on to next host if it was a connection error
        if not (err and err.startswith('connection_refused')):
            break

    # All hosts failed — build a helpful message
    timeout_hit = any('timeout' in str(v) for v in last_errors.values())
    conn_refused = all('connection_refused' in str(v) for v in last_errors.values())

    if conn_refused:
        msg = (
            f'Cannot connect to Ollama on any of: {list(last_errors.keys())}. '
            'Run these two commands in separate terminals, then retry:\n'
            '  1)  ollama serve\n'
            f'  2)  ollama run {model}'
        )
    elif timeout_hit:
        msg = (
            f'Ollama timed out (model: {model}). '
            'The model may be loading for the first time or your GPU is busy. '
            'Wait ~30 s and try again. To pre-load: ollama run ' + model
        )
    else:
        detail = '; '.join(f'{h}: {e}' for h, e in last_errors.items())
        msg = f'Ollama error — {detail}'
